...
const LS_SNAPSHOT = 'battydev.brackets.snapshot.v2';
const LS_SNAPSHOT_BACKUP = 'battydev.brackets.snapshot.backup.v2';
...
function persistNow() {
  ...
  const previous = readStorage(LS_SNAPSHOT);
  if (previous.error) errors.push(previous.error);
  else if (previous.value) {
    const error = writeStorage(LS_SNAPSHOT_BACKUP, previous.value);
  ...
  const snapshotError = writeStorage(LS_SNAPSHOT, snapshotRaw);
  if (snapshotError) {
    setStorageIssue(snapshotError);
    return false;
  }

  for (const [key, value] of [[LS_STATE, stateRaw], [LS_QUEUE, queueRaw]]) {
...
function hydrate() {
  const storedState = readStorage(LS_STATE);
  const storedQueue = readStorage(LS_QUEUE);
  const storedSnapshot = readStorage(LS_SNAPSHOT);
  const storedBackup = readStorage(LS_SNAPSHOT_BACKUP);
...
function deviceId() {
  const stored = readStorage('battydev.brackets.device');
  let id = stored.value;
  if (!id) {
    id = uid('dev');
    const error = writeStorage('battydev.brackets.device', id);
...
export async function drain() {
  if (syncing || !client || !online || !queue.some((op) => !op.blocked && !op.requiresReview)) return;
  syncing = true;
  ...
  while (queue.some((op) => !op.blocked && !op.requiresReview)) {
    ...
    try {
      let result;
      if (op.patch === null) {
        result = await client.from(table).delete().eq('id', op.id);
      } else {
        ...
        result = await client.from(table).upsert({ ...rowFor(op.patch), id: op.id, updated_at: op.at });
      }
...
export function reset() {
  ...
  queue = [];
  undoStack.length = 0;
  for (const key of [LS_STATE, LS_QUEUE, LS_SNAPSHOT, LS_SNAPSHOT_BACKUP]) {
