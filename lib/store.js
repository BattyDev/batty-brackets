...
export async function selfCheckInRemote(eventId) {
  if (!remoteBackend) throw new Error('No connected backend is available.');
  const result = await remoteBackend.selfCheckIn(eventId);
  cacheRemote({ entries: [result.entry] }, { silent: true });
  requestPersist();
  notify({ type: 'remote-self-checkin', eventId, entryId: result.entry.id, changed: result.changed });
  return result.entry;
}

export async function signDocumentRemote(eventId, documentId, documentVersion, typedName) {
  if (!remoteBackend) throw new Error('No connected backend is available.');
  const result = await remoteBackend.signDocument(eventId, documentId, documentVersion, typedName);
  cacheRemote({ entries: [result.entry] }, { silent: true });
  requestPersist();
  notify({ type: 'remote-sign-document', eventId, entryId: result.entry.id, documentId, changed: result.changed });
  return result.entry;
}
