...
export function boot({ demo = false } = {}) {
  ...
  /* Seed the demo the first time only, and never over real data. Someone who
     has run an actual event on this device must not have it filled with
     fictional players because they opened it while the backend was down. */
  if (demo && !Object.keys(state.events).length && !Object.keys(state.players).length) {
    seedDemoData({ apply, uid, inviteCode });
    persist();
  }
