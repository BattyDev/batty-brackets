...
export function apply(collection, id, patch, { queueIt = true, silent = false } = {}) {
  ...
  if (queueIt && remoteBackend) {
    /* Connected mode has no generic record replay. Existing excluded views
       may still use apply() for local-only controls; retain the in-memory
       change for that view but make it impossible to enqueue or upload it. */
    localOnlyWrites += 1;
    lastServerError = 'This connected control is local-only until its server command is implemented.';
  } else if (queueIt) {
    queue.push({
...
export const syncState = () => ({
  ...
  remoteWrites: Boolean(remoteBackend) ? false : Boolean(legacyClient),
  localOnlyWrites,
  scope: storageScope(),
  online,
...
const REMOTE_CACHE_COLLECTIONS = new Set(['players', 'orgs', 'events', 'entries', 'stations']);
