...
async function adoptSupabaseSession(sb) {
  ...
  session = {
    ...
    temporary: user.is_anonymous === true,
...
export async function signInWithDiscord() {
  ...
  if (session?.temporary) throw new Error('Save this guest record with the account upgrade option so your tournament history stays attached.');
...
export async function signUpWithEmail(email, password, tag, captchaToken) {
  if (!client) return signInLocal({ tag: tag || email.split('@')[0], email, via: 'email' });
  if (session?.temporary) throw new Error('Save this guest record with the account upgrade option so your tournament history stays attached.');

  const { data, error } = await client.auth.signUp({
    email, password,
    options: { data: { full_name: tag || '' }, captchaToken },
  });
  if (error) throw friendlyAuthError(error);
  return data;
}
...
export async function signInWithEmail(email, password, captchaToken) {
  ...
  if (session?.temporary) throw new Error('Signing into a different account would split this guest record. Save this guest as a new account first.');
...
export async function createTemporaryPlayer({ tag, captchaToken } = {}) {
  const clean = String(tag || '').trim().replace(/\s+/g, ' ');
  if (!clean || clean.length > 32) throw new Error('Enter a nickname up to 32 characters.');
  if (!client) {
    const next = signInLocal({ tag: clean, via: 'guest' });
    next.temporary = true;
    setSession(next);
    emit();
    return next;
  ...
  if (session) {
    if (!session.temporary) throw new Error('You are already signed in. Join with your existing player profile.');
    return session;
  }
  if (typeof client.auth.signInAnonymously !== 'function') throw new Error('Temporary player accounts are not enabled on this server. Sign in to continue.');
  const { data, error } = await client.auth.signInAnonymously({
    options: { data: { full_name: clean }, captchaToken },
  });
  if (error) throw friendlyAuthError(error);
  if (!data?.session?.user) throw new Error('The server did not create a temporary player session.');
  return adoptSupabaseSession(data.session);
}

export async function upgradeTemporaryWithDiscord() {
  if (!client || !session?.temporary) throw new Error('There is no guest record to save.');
  if (typeof client.auth.linkIdentity !== 'function') throw new Error('Discord account linking is not enabled on this server.');
...
export async function upgradeTemporaryWithEmail(email, tag) {
  if (!client || !session?.temporary) throw new Error('There is no guest record to save.');
  const cleanEmail = String(email || '').trim().toLowerCase();
  const cleanTag = String(tag || currentPlayer()?.tag || '').trim();
  if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(cleanEmail)) throw new Error('Enter a valid email address.');
  const { data, error } = await client.auth.updateUser({
    email: cleanEmail,
    data: { full_name: cleanTag, brackets_guest_upgrade: true },
  }, { emailRedirectTo: `${window.location.origin}${window.location.pathname}` });
  if (error) throw friendlyAuthError(error);
  if (session) {
    session.pendingUpgradeEmail = cleanEmail;
    setSession(session);
    emit();
  }
