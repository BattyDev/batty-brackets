...
import { apply, get, uid, getPlayer } from './store.js';
...
export async function initAuth(supabaseClient) {
  ...
  client.auth.onAuthStateChange(async (event, next) => {
    if (event === 'SIGNED_OUT') { session = null; apply('session', 'session', null, { queueIt: false }); emit(); return; }
    if (next) await adoptSupabaseSession(next);
  });

  return session;
}

/* Turn a Supabase session into our session + player row.

   The player row is keyed on the auth user id, so signing in a second way --
   adding a password to a Discord account, say -- lands on the SAME player,
   because Supabase links both identities under one user. That is the whole
   mechanism behind "one person, many logins": we do not implement linking, we
   let the auth provider do it and key off the thing that survives it. */
async function adoptSupabaseSession(sb) {
  const user = sb.user;
  const discord = user.identities?.find((i) => i.provider === 'discord');
  const meta = user.user_metadata || {};

  const existing = getPlayer(user.id);
  const player = {
    id: user.id,
    tag: existing?.tag || meta.custom_claims?.global_name || meta.full_name || meta.name
    ...
    methods: (user.identities || []).map((i) => i.provider),
    createdAt: existing?.createdAt || user.created_at,
    claimable: false,
    claimCode: null,
  };
  apply('players', player.id, player);
  ...
  session = {
    ...
    needsPasswordFallback: player.methods.includes('discord') && !player.methods.includes('email'),
    local: false,
  };
  apply('session', 'session', session, { queueIt: false });
  emit();
  return session;
}
...
export async function signInWithDiscord() {
  if (!client) return signInLocal({ tag: 'Local TO', via: 'discord' });
...
export async function methodsFor(email) {
  ...
  try {
    const { data, error } = await client.rpc('bkt_auth_methods', { p_email: clean });
    if (error) return [];
    return data || [];
  } catch {
    /* Offline, or the RPC is not deployed. Returning "unknown" rather than
       "none" matters: the caller must not tell the user "this email is free"
       on the strength of a failed lookup and then hit a duplicate error. */
    return null;
  }
}
...
export async function claim(code) {
  ...
  if (client) {
    const { error } = await client.rpc('bkt_claim_player', { p_code: code, p_into: me.id });
