...
const TV_STEPS = [
  {
    ...
    /* 14, chosen by measurement rather than taste. The two display screens
       pull in opposite directions: playing more deepens the bracket and
       drains the queue. At 8 the queue is full and the bracket has not left
       round two; at 24 the bracket reaches grand finals and there is nothing
       left to play, so the "who is up" screen is empty. 14 leaves two sets on
       stations, three waiting, and the bracket into the quarter-finals --
       both screens showing something real. */
    act() { generateAndPlay(14, { stations: 3 }); },
  },
  {
    id: 'tv-queue',
...
function generateAndPlay(setsToPlay, { stations: stationCount = 2 } = {}) {
  const event = store.getEvent(DEMO_EVENT);
  ...
  /* Some sets left out on stations, so the run view and the display both have
     live timers rather than a screen of idle hardware. */
  const stations = store.stationsFor(DEMO_EVENT).filter((s) => !s.closed);
  const stillReady = readyMatches(matches);
  stations.slice(0, stationCount).forEach((station, i) => {
    const match = stillReady[i];
    if (!match) return;
    matches = matches.map((m) => (m.id === match.id
      /* Staggered ages so one of them is past the DQ window and the display
         has a red timer to show -- that state is half the reason the screen
         exists and a demo that never reaches it undersells it. */
      ? { ...m, calledAt: new Date(Date.now() - [9, 2, 4][i % 3] * 60_000).toISOString(), stationId: station.id }
      : m));
